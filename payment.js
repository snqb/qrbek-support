...
export const BANKS = Object.freeze([
  {
    id: "mbank",
    name: "MBANK",
    prefix: "https://app.mbank.kg/qr/#",
    icon: "/assets/banks/mbank.png",
    allowAsSource: true,
  },
  {
    id: "bakai",
    name: "Bakai Bank",
    prefix: "https://bakai.app/#",
    icon: "/assets/banks/bakai.png",
    allowAsSource: true,
  },
  {
    id: "optima",
    ...
    icon: "/assets/banks/optima.png",
    allowAsSource: true,
  },
  {
    id: "demir",
    name: "DemirBank",
    prefix: "https://retail.demirbank.kg/#",
    icon: "/assets/banks/demir.png",
    allowAsSource: true,
  },
  {
    id: "odengi",
    name: "О! Деньги",
    prefix: "https://api.dengi.o.kg/#",
    icon: "/assets/banks/odengi.png",
    allowAsSource: true,
  },
  {
    id: "megapay",
    name: "MegaPay",
    prefix: "https://megapay.kg/get#",
    icon: "/assets/banks/megapay.png",
    allowAsSource: true,
  },
  {
    id: "eldik",
    name: "Eldik Bank",
    prefix: "https://app.eldik.kg/#",
    icon: "/assets/banks/eldik.png",
    allowAsSource: true,
  },
  {
    id: "simbank",
    name: "Simbank",
    prefix: "https://simbank.kg/scan_to_pay?payload=#",
    icon: "/assets/banks/simbank.png",
    allowAsSource: true,
  },
  {
    id: "kompanion",
    name: "Компаньон",
    prefix: "https://24.kompanion.kg/qr/#",
    icon: "/assets/banks/kompanion.png",
    allowAsSource: true,
  },
  {
    id: "kicb",
    name: "KICB",
    prefix: "https://bank.kicb.net/#",
    icon: "/assets/banks/kicb.png",
    allowAsSource: true,
  },
  {
    id: "nambaone",
    name: "NambaOne",
    prefix: "https://nambaone.app/#",
    icon: "/assets/banks/nambaone.png",
    allowAsSource: true,
  },
  {
    id: "balance",
    name: "Balance",
    prefix: "https://balance.kg/#",
    icon: "/assets/banks/balance.png",
    allowAsSource: true,
  },
  {
    id: "ab24",
    name: "АБ24",
    prefix: "https://qr.ab.kg/#",
    icon: "/assets/banks/ab24.png",
    allowAsSource: true,
  },
  {
    id: "dantepay",
    name: "DantePay",
    prefix: "https://dantepay.kg/elqr/form?qrLink=#",
    icon: "/assets/banks/dantepay.png",
    allowAsSource: true,
  },
  {
    id: "finik",
    name: "Finik",
    prefix: "https://qr.finik.kg/#",
    icon: "/assets/banks/finik.png",
    allowAsSource: true,
  },
  {
    id: "bankasia",
    name: "Bank of Asia",
    prefix: "https://www.bankasia.kg/#",
    icon: "/assets/banks/bankasia.png",
    allowAsSource: false,
  },
]);
...
function normalizeAmount(value, field = "сумма", allowZero = false) {
  ...
  if (!/^(?:0|[1-9][0-9]{0,8})(?:\.[0-9]{1,2})?$/.test(value)) {
    error(
      `${field}: используйте десятичное число с точкой и не более 2 знаков после неё`,
    );
  }
  const [whole, fraction = ""] = value.split(".");
  const cents = BigInt(whole) * 100n + BigInt((fraction + "00").slice(0, 2));
  if (cents < (allowZero ? 0n : 1n) || cents > 99_999_999_999n) {
    error(
      `${field} должна быть от ${allowZero ? "0" : "0.01"} до 999999999.99 KGS`,
    );
  }
  return value;
}
...
function assertPlainObject(value, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    error(`${label} должна быть объектом`);
  }
}
...
export function normalizePage(input) {
  ...
  if (codePointCount(input.title) > 80) {
    error("заголовок слишком длинный (максимум 80 символов)");
  }
  if (codePointCount(input.note) > 240) {
    error("примечание слишком длинное (максимум 240 символов)");
  }
  if (input.currency !== "KGS") error("поддерживается только валюта KGS");
  const amount = normalizeAmount(input.amount);
  if (
    !Array.isArray(input.methods) || input.methods.length < 1 ||
    input.methods.length > 8
  ) {
  ...
  const methods = input.methods.map((method, index) => {
    ...
    const extra = Object.keys(method).filter((key) =>
      !methodFields.includes(key)
    );
    if (extra.length) {
      error(`способ оплаты ${index + 1}: неизвестное поле ${extra[0]}`);
    }
    if (method.kind !== "qr") {
      error(`способ оплаты ${index + 1}: поддерживаются только QR`);
    }
    if (typeof method.label !== "string" || codePointCount(method.label) > 80) {
      error(`способ оплаты ${index + 1}: название до 80 символов`);
    }
    if (
      typeof method.value !== "string" || method.value.trim() === "" ||
      utf8Size(method.value) > 3000
    ) error(`способ оплаты ${index + 1}: непустое значение до 3000 байт`);
    if (
      typeof method.bankId !== "string" ||
      (method.bankId !== "" && !bankById.has(method.bankId))
    ) {
      error(`способ оплаты ${index + 1}: неизвестный банк`);
    }
    const identity =
      `${method.kind}\u0000${method.value}\u0000${method.bankId}`;
    if (seenMethods.has(identity)) {
      error(`способ оплаты ${index + 1}: дубликат`);
    }
...
function base64UrlToBytes(value) {
  ...
  try {
    ...
    if (caught instanceof Error && caught.message.startsWith("Ошибка:")) {
      throw caught;
    }
    error("некорректная base64url-ссылка");
  }
...
export function decodePage(encoded) {
  ...
  if (value.length > Math.ceil(MAX_JSON_BYTES / 3) * 4) {
    error("страница слишком большая");
  }
  if (value.includes("#") || value.includes("?") || value.includes("/")) {
    error("некорректный фрагмент страницы");
  }
...
function decodeFragmentOnce(value) {
  ...
  for (let i = 0; i < value.length;) {
    ...
    while (i < value.length && value[i] === "%") {
      ...
      ) error("QR содержит некорректное URL-кодирование");
...
function parseTlv(data) {
  ...
  while (index < data.length) {
    if (
      data.length - index < 4 ||
      !/^[0-9]{4}$/.test(data.slice(index, index + 4))
    ) return null;
...
function merchantBank(merchantId) {
  ...
  for (const [id, aliases] of merchantAliases) {
    for (const alias of aliases) {
      if (
        alias && text.includes(alias.toLowerCase()) &&
        alias.length > selectedLength
      ) {
...
function sbpUrl(value) {
  ...
  return parsed.protocol === "https:" && parsed.hostname === "qr.nspk.ru" &&
    parsed.port === "" && parsed.username === "" && parsed.password === "" &&
    parsed.search === "" && parsed.hash === "" &&
    /^\/A[SD][A-Za-z0-9_-]{6,200}$/.test(parsed.pathname);
}
...
async function sha256Hex(value) {
  if (!globalThis.crypto?.subtle) {
    error("в этой среде недоступен crypto.subtle");
  }
...
async function checksumInfo(payload, parsed) {
  ...
  if (
    parsed.fields[parsed.fields.length - 1] !== crcField ||
    crcField.value.length !== 4 || !/^[0-9A-Fa-f]{4}$/.test(crcField.value)
  ) {
  ...
  if (deployed === supplied) {
    return { valid: true, scheme: "sha256", crc: supplied };
  }
...
function merchantData(parsed) {
  ...
  const merchantField = parsed.fields.find(({ tag }) =>
    tag !== "34" && Number(tag) >= 26 && Number(tag) <= 51
  );
  let nested = null;
  if (merchantField) {
  ...
  const national = merchantField?.tag === "32"
    ? nested
    : top.has("32")
    ? parseTlv(top.get("32"))
    : null;
  if (top.has("32") && !national) return null;
  const editPermission = national?.byTag.get("12");
  ...
  const score = (value) => {
    ...
    const letters =
      Array.from(value).filter((char) => /\p{L}/u.test(char)).length;
    const words = value.trim().split(/\s+/).length;
    return letters + (words >= 2 ? 10 : 2) - (/[0-9]/.test(value) ? 14 : 0) -
      (/(bank|банк|mbank|o!bank|demir|bakai|optima|megapay|eldik|simbank|kompanion|dantepay)/i
          .test(normalized)
        ? 20
        : 0);
  };
  const name = names.length
    ? names.reduce((best, candidate) =>
  ...
  return {
    ...
    account,
    name,
    amountInMinorUnits: Boolean(national),
    amountEditable: editPermission === undefined || editPermission === "11",
  };
}
...
function parseElqrSource(value) {
  ...
  if (!trimmed.startsWith("0002")) {
    ...
    if (!wrapper) {
      error("разрешён только известный URL банка Кыргызстана или raw ELQR");
    }
    const hash = trimmed.indexOf("#");
    payload = decodeFragmentOnce(trimmed.slice(hash + 1));
  }
  if (!payload.startsWith("0002") || utf8Size(payload) > MAX_QR_BYTES) {
    error("некорректный ELQR payload");
  }
  const parsed = parseTlv(payload);
  if (!parsed || parsed.fields.length === 0) {
    error("ELQR содержит неполный или повреждённый TLV");
  }
  const currency = parsed.byTag.get("53");
  if (currency && currency !== "417") error("QR не в валюте KGS");
  const country = parsed.byTag.get("58");
  if (country && country.toUpperCase() !== "KG") {
    error("QR не относится к Кыргызстану");
  }
  const details = merchantData(parsed);
  if (!details) error("ELQR содержит повреждённые данные получателя");
  let amount = parsed.byTag.get("54") ?? null;
  if (amount !== null) {
    if (amount === "") error("сумма в QR пуста");
    // NBKR national template 32 uses integer tyiyn; legacy EMV uses major units.
    // Bank-issued zero placeholders are accepted, but never requested as payments.
    if (details.amountInMinorUnits) {
      if (!/^(?:0(?:\.0{1,2})?|[1-9][0-9]{0,10})$/.test(amount)) {
        error("сумма в ELQR должна быть целым числом тыйынов");
      }
      const tyiyn = amount.includes(".") ? 0n : BigInt(amount);
      amount = `${tyiyn / 100n}.${String(tyiyn % 100n).padStart(2, "0")}`;
    }
    normalizeAmount(amount, "сумма в QR", true);
  }
  ...
  return { trimmed, payload, parsed, wrapper, routeBank, details, amount };
}

function canRewriteAmount(source) {
  const initiation = source.parsed.byTag.get("01");
  return (source.amount === null || amountCents(source.amount) === 0n) &&
    source.details.amountEditable && !source.parsed.byTag.has("64") &&
    (!initiation || initiation === "11");
}

export async function inspectQr(value) {
  if (typeof value !== "string" || value.trim() === "") {
    error("QR должен быть непустой строкой");
  }
  const trimmed = value.trim();
  if (sbpUrl(trimmed)) {
    return {
  ...
  return {
    ...
    mutableAmount: canRewriteAmount(source),
    kind: "elqr",
  };
}
...
export async function paymentTarget(value, amount = "", sourceBankId = "") {
  ...
  if (sbpUrl(trimmed)) {
    return {
      ...
      amountApplied: false,
      notice: requested
        ? "СБП не поддерживает изменение суммы; сумма указана только информационно"
        : "",
    };
  }
  const source = parseElqrSource(trimmed);
  ...
  if (sourceBankId !== "") {
    ...
    if (!selectedBank.allowAsSource) {
      error("этот банк нельзя выбрать как банк оплаты");
    }
  }
  const targetUrl = (payload) => wrapPayload(payload, selectedBank);
  const qrTextFor = (payload, url, preserveOriginal = false) =>
    preserveOriginal
      ? (trimmed.startsWith("0002") ? payload : (sourceBankId ? url : trimmed))
      : (trimmed.startsWith("0002") ? payload : url);
  const existing = source.amount;
  if (!requested) {
    const url = trimmed.startsWith("0002") || sourceBankId
      ? targetUrl(source.payload)
      : trimmed;
    return {
      qrText: qrTextFor(source.payload, url, true),
      url,
      amount: amountCents(existing ?? "") === 0n ? "" : existing ?? "",
      amountApplied: false,
      notice: "",
    };
  }
  ...
  if (!canRewriteAmount(source)) {
    error(
      "этот QR нельзя изменить: сумма фиксирована, QR динамический/подписанный или банк запретил изменение",
    );
  }
  const rewritten = await replaceAmountAsync(
    source.payload,
    source,
    requested,
    checks.scheme,
  ...
  return {
    qrText: qrTextFor(rewritten, url),
    url,
    amount: requested,
    amountApplied: true,
    notice: "",
...
async function replaceAmountAsync(payload, source, amount, scheme) {
  const checksumField = source.parsed.fields.find((field) =>
    field.tag === "63"
  );
  const bodyEnd = checksumField ? checksumField.start : payload.length;
  const existingField = source.parsed.fields.find((field) =>
    field.tag === "54"
  );
  const insertionField = existingField ||
    source.parsed.fields.find((field) =>
      field.tag !== "63" && Number(field.tag) > 54
    );
  const insertion = insertionField ? insertionField.start : bodyEnd;
  const encodedAmount = source.details.amountInMinorUnits
    ? amountCents(amount).toString()
    : amount;
  const amountFieldText = `54${
    String(encodedAmount.length).padStart(2, "0")
  }${encodedAmount}`;
  const withAmount = payload.slice(0, insertion) + amountFieldText +
    payload.slice(existingField?.end ?? insertion, bodyEnd);
  if (scheme === "sha256") {
    return `${withAmount}6304${(await sha256Hex(withAmount)).slice(-4)}`;
  }
